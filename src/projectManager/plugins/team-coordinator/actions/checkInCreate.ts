  ...
  handler: async (
    ...
    try {
      ...
      try {
        ...
        logger.error(
          `Failed to get Discord client: ${discordError.message || "Unknown error"}`,
        );
      ...
      if (discordService?.client) {
        try {
          logger.info(
            `Fetching all channels from Discord server with ID: ${serverId}`,
          );
          ...
          if (guild) {
            ...
            logger.info(
              `Found ${channels.size} channels in server ${guild.name}`,
            );
            ...
            logger.info(
              `Stored ${textChannels.length} text channels for forms`,
            );
      ...
      // Get message source
      const messageSource =
        (message.content.source as string) || room.source || "unknown";
      ...
      const parsedQuestionResponse = await runtime.useModel(
      ...
      logger.info(
        "Raw AI response for question check:",
      ...
      if (parsedQuestionResponse.trim().toLowerCase() === "true") {
        logger.info(
          "User is asking a question about check-ins, not providing configuration",
        );
        ...
        if (!existingConfig) {
          ...
          logger.debug(
            `Using existing config: ${JSON.stringify(existingConfig)}`,
          );
      ...
      logger.info(
        "User is providing check-in configuration, proceeding to parse",
      );
      ...
      try {
        ...
        logger.info(
          "Successfully parsed check-in configuration:",
          checkInConfig,
        );
      } catch (error: unknown) {
      ...
      const checkInChannelName =
        checkInConfig.channelForCheckIns?.toLowerCase();
      ...
      if (!existingConfig) {
        logger.info(
          "No existing report channel config found, creating new one",
        );
        try {
      ...
      try {
        ...
        const schedule: CheckInSchedule = {
          ...
          frequency: (checkInConfig.frequency ||
        ...
        logger.info(
          `Using existing room with ID: ${roomId} for check-in schedules`,
        );
        ...
        try {
          ...
          logger.error(
            `Failed to ensure room exists: ${err.message || "Unknown error"}`,
          );
        ...
        const scheduleMemory = {
          id: createUniqueUuid(
            runtime,
            `checkin-schedule-${schedule.scheduleId}-${Date.now()}`,
          ),
          entityId: runtime.agentId,
          agentId: runtime.agentId,
          content: {
