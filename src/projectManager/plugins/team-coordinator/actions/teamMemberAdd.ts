...
async function fetchTeamMembersForServer(
  ...
  try {
    ...
    // Log for debugging
    logger.info(
      `Successfully retrieved ${teamMembers.length} team members for server ${serverId}`,
    );
...
export const addTeamMemberAction: Action = {
  ...
  validate: async (
    ...
    try {
      ...
      logger.info(
        `Checking if user ${message.entityId} is an admin for server ${serverId}`,
      );
      ...
      const err = error as Error;
      logger.error(
        `Error in addTeamMemberAction validation: ${toErrorMessage(error)}`,
      );
      logger.error(`Error stack: ${err.stack || "No stack trace available"}`);
      return false;
    }
  ...
  handler: async (
    ...
    try {
      ...
      if (!state) {
        return { success: false, error: "Missing state for team member add" };
      }
      ...
      logger.info(
        `Processing team members for server: ${serverId} (${serverName})`,
      );
      ...
      try {
        ...
        try {
          ...
          logger.info(
            `Successfully parsed team member configuration: ${stringifyForLog(teamMembers)}`,
          );
        } catch (parseError) {
          logger.error(
            `Failed to parse AI response as JSON: ${toErrorMessage(parseError)}`,
          );
        ...
        const validatedTeamMembers = teamMembers.filter(
          (member: TeamMember) => {
        ...
        if (validatedTeamMembers.length === 0) {
          ...
          return {
            success: false,
            error: "No valid team members found after validation",
          };
        }
        ...
        if (!existingConfig) {
          logger.info(
            "No existing store-team-members-memory found, creating new one",
          );
          try {
            ...
            logger.info(
              `Creating store-team-members channel config: ${stringifyForLog(config)}`,
            );
            ...
            try {
              ...
              logger.error(
                `Room error stack: ${roomError.stack || "No stack trace available"}`,
              );
            }
            ...
            logger.error(
              `Error stack: ${configError.stack || "No stack trace available"}`,
            );
          }
        } else {
          logger.info(
            "Found existing team members config, checking for new members to add",
          );
          ...
          const newTeamMembers = validatedTeamMembers.filter(
            (newMember) =>
              !isDuplicateTeamMember(existingTeamMembers, newMember),
          );

          if (newTeamMembers.length === 0) {
            logger.info(
              "No new team members to add, all are already registered",
            );
            ...
            logger.info(
              `Adding ${newTeamMembers.length} new team members to existing config`,
            );
        ...
        logger.error(
          `Failed to parse team member information: ${toErrorMessage(error)}`,
        );
        logger.error(
          `Error stack: ${parsingError.stack || "No stack trace available"}`,
        );
        ...
        return {
          success: false,
          error: "Failed to parse team member information",
        };
      }
      ...
      logger.error(
        `Error processing team member recording: ${toErrorMessage(error)}`,
      );
      logger.error(`Error stack: ${err.stack || "No stack trace available"}`);
