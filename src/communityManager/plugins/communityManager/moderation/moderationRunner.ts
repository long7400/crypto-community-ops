...
import { formatCommunityModerationSettingsSummary, parseTelegramModerationAdminCommand } from "./adminConfig";
...
export class ModerationRunner {
	...
	async handleMemory(memory: Memory): Promise<void> {
		...
		const settings = await settingsRepository.getForWorld(message.worldId as any);
		...
		const state = await violationStore.getState({
			...
			category: classification.category === "safe" ? "spam" : classification.category,
		});
