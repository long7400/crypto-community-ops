...
export class AgentLoadTestSuite implements TestSuite {
  ...
  private readonly SCALE_CONFIGS: ScaleConfig[] = [
    ...
    { agents: 50, messageCount: 100, description: "Large Group (50 agents)" },
    {
      agents: 100,
      messageCount: 150,
      description: "Very Large Group (100 agents)",
    },
    { agents: 250, messageCount: 200, description: "Mass Scale (250 agents)" },
    {
      agents: 500,
      messageCount: 300,
      description: "Extreme Scale (500 agents)",
    },
    {
      agents: 1000,
      messageCount: 400,
      description: "Maximum Load (1000 agents)",
    },
    {
      agents: 2000,
      messageCount: 500,
      description: "Breaking Point (2000 agents)",
    },
  ];
  ...
  tests = [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const summaryLogFile = path.join(
          logsDir,
          `load-test-summary-${timestamp}.log`,
        );
        const systemMetricsFile = path.join(
          logsDir,
          `system-metrics-${timestamp}.log`,
        );
        ...
        for (const thresholdLevel of thresholdLevels) {
          ...
          for (const config of this.SCALE_CONFIGS) {
            ...
            try {
              ...
              fs.writeFileSync(
                detailedLogFile,
                createDetailedTestLogHeader(config),
              );
              fs.appendFileSync(
                detailedLogFile,
              ...
              const metrics = await this.runSequentialLoadTest(
                runtime,
                config,
                thresholdLevel,
              );
              ...
              fs.appendFileSync(
                detailedLogFile,
                formatDetailedMetrics(config, metrics),
              );
        ...
        const optimalConfigs =
          this.determineOptimalScalingPerThreshold(summaryLogFile);
  ...
  private async runSequentialLoadTest(
    ...
    try {
      ...
      const roomId = await this.scenarioService.createRoom(
        worldId,
        `load-test-${config.agents}`,
      );

      // Add the test agent
      await this.scenarioService.addParticipant(
        worldId,
        roomId,
        runtime.agentId,
      );
      ...
      for (let i = 0; i < config.messageCount; i++) {
        ...
        try {
          ...
          try {
            const completed =
              await this.scenarioService.waitForCompletion(10000);
            if (!completed) {
              metrics.timeoutCount++;
              metrics.errorTypes["timeout"] =
                (metrics.errorTypes["timeout"] || 0) + 1;
            ...
            if (responseTime > thresholds.responseTimeThreshold) {
              ...
              metrics.errorTypes["slow_response"] =
                (metrics.errorTypes["slow_response"] || 0) + 1;
              if (metrics.errorTypes["slow_response"] > 5) {
                throw new Error(
                  `Too many slow responses detected (>${responseTime}ms)`,
                );
              }
            }
          } catch (timeoutError) {
        ...
        if (i % 10 === 0 || i === config.messageCount - 1) {
          ...
          if (
            ...
            this.systemMetrics.cpuUsage[
              this.systemMetrics.cpuUsage.length - 1
            ...
            metrics.errorTypes["resource_limit"] =
              (metrics.errorTypes["resource_limit"] || 0) + 1;
    ...
    metrics.successRate =
      metrics.messagesSent > 0
        ? (metrics.messagesProcessed / metrics.messagesSent) * 100
        : 0;
    metrics.throughput =
      metrics.messagesProcessed > 0
        ? (metrics.messagesProcessed / metrics.totalTime) * 1000
        : 0; // messages per second
  ...
  private determineOptimalScalingPerThreshold(summaryFile: string): Record<
    ThresholdLevel,
    {
      agents: number;
      messageCount: number;
    ...
    try {
      ...
      for (const level of Object.keys(ThresholdLevel)) {
        ...
        const rows = summary
          ...
          .filter(
            (line) => !line.includes("FAILED") && !line.includes("SKIPPED"),
          );
        ...
        if (rows.length > 0) {
          ...
          const description = fullDescription
            .substring(fullDescription.indexOf("-") + 1)
            .trim();
  ...
  private reportMetrics(
    ...
    logger.info("\n========== Test Results ==========");
    logger.info(
      `Configuration: ${config.description} (${thresholdLevel} threshold)`,
    );
    logger.info(`Number of Agents: ${config.agents}`);
    logger.info(`Messages Sent: ${metrics.messagesSent}`);
    logger.info(
      `Messages Successfully Processed: ${metrics.messagesProcessed}`,
    );
