...
try {
  ...
  if (summaryFiles.length > 0) {
    ...
    if (conclusions) {
      ...
      fs.appendFileSync(
        mainLogFile,
        utils.formatScalabilityConclusions(conclusions),
      );
    }
  }
  ...
  logger.info(
    "Check the logs directory for detailed test results for each configuration.",
  );
} catch (error) {
  ...
  logger.error(
    "Tests encountered errors, which may indicate the breaking point was reached.",
  );
