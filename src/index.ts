...
function hasRequiredEnvVars(
  agent:
    | ProjectAgent
    | { character: Partial<Character>; init: (runtime: any) => Promise<void> },
): boolean {
  // Get which platform plugins the agent uses
  const usesDiscord = agent.character.plugins?.includes(
    "@elizaos/plugin-discord",
  );
  const usesTelegram = agent.character.plugins?.includes(
  ...
  if (usesDiscord) {
    ...
    const discordId =
      typeof secrets === "object" && secrets !== null
    ...
    if (discordId && discordToken) {
      ...
      logger.debug(
        `Agent "${agent.character.name || "Unknown"}" has Discord configuration`,
      );
    }
  }
  ...
  if (usesTelegram) {
    ...
    if (telegramToken) {
      hasValidPlatform = true;
      logger.debug(
        `Agent "${agent.character.name || "Unknown"}" has Telegram configuration`,
      );
    }
  }
  ...
  if (!hasValidPlatform) {
    logger.warn(
      `Agent "${agent.character.name || "Unknown"}" disabled - missing platform configuration`,
    );
  }
...
if (doubleDashIndex !== -1) {
  ...
  potentialAgentFlags = rawArgs
    .slice(doubleDashIndex + 1)
  ...
  potentialAgentFlags = rawArgs.filter(
...
if (potentialAgentFlags.length > 0) {
  ...
  const matchedAgents = allAgents.filter((agent) => {
    ...
    return agentKey
      ? requestedAgentNames.includes(agentKey.toLowerCase())
      : false;
  });
  ...
  if (matchedAgents.length > 0) {
    ...
    allAgents.forEach((agent) => {
      ...
      if (objectKey) {
        logger.warn(
          `  --${objectKey} (for ${agent.character.name || "Unknown"})`,
        );
      }
    });
